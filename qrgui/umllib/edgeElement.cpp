		...
		, mModelUpdateIsCalled(false)
{
	...
	connect(&mSquarizeAction, SIGNAL(triggered(QPointF const &)), SLOT(setGraphicApi(QPointF const &)));
	connect(&mMinimizeAction, SIGNAL(triggered(QPointF const &)), SLOT(minimizeHandler(QPointF const &)));
	connect(&mDelSegmentAction, SIGNAL(triggered(QPointF const &)), SLOT(deleteSegment(QPointF const &)));
...
void EdgeElement::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
	...
	if ((mSrc && mDst && mSrc->isSelected() && mDst->isSelected() && isSelected())
		|| (mSrc && (mDst == NULL) && mSrc->isSelected() && isSelected())
		|| (mDst && (mSrc == NULL) && mDst->isSelected() && isSelected()))
	{
		mDragPoint = -1;
...
{
	int const delWidth = kvadratik * 3;
...
void EdgeElement::delPointHandler(QPointF const &pos)
{
	...
	if (pointIndex != -1 && pointIndex != mLine.count() - 1 && pointIndex) {
		...
		updateLongestPart();
		//mGraphicalAssistApi->setConfiguration(id(), mLine.toPolygon()); // сообщаем об изменени€х на "верхних" уровн€х
		update();
	}
}
	...
	while (i + 1 < mLine.size()) {
		...
		if (i < mLine.size() - 3) {
			if (mLine[i + 1].x() == mLine[i].x() && mLine[i + 2].x() == mLine[i].x()) {
				mLine.remove(i + 1);
				i++; // так как может быть несколько точек подр€д, то лучше убрать, но они удал€ютс€ при adjust
				continue;
			}
...
void EdgeElement::adjustLink()
{
	...
	//setGraphicApi(QPointF()); //¬ј∆Ќќ: если оставить, будет лагать при переносе многих элементов (вместе с линками).
	//если убрать, обновлени€ будут при mouseRelease, не при mouseMove
...
void EdgeElement::setColorRect(bool bl) // "пуста€" функци€
{
	Q_UNUSED(bl);
}
...
QPointF* EdgeElement::haveIntersection(QPointF const &pos1, QPointF const &pos2, QPointF const &pos3, QPointF const &pos4)
{
	...
	qreal y = (-c1 + a1 * c2 / a2)/(-(a1 / a2) * b2 + b1);
	qreal x = (b2 * y + c2) / (-a2);
	QPointF* cut = new QPointF(x,y);
	// use class QPainterPath for determine of the accessory
	QPainterPath path;
	QPainterPathStroker ps;
	ps.setWidth(1);
	path.moveTo(pos1);
	...
	for (int i = startPos; i < mLine.size() - 3; ++i)
	{
		...
		for (int j = i + 2; j < mLine.size() - 1; ++j)
		{
			...
			if (cut != NULL)
			{
				...
				{
					addPointHandler(QPointF(cut->x(),cut->y()));
...
void EdgeElement::deleteSegment(QPointF const &pos)
{
	...
	for (int i = 0; i < mLine.size() - 1; ++i) {
		path.moveTo(mLine[i]);
		path.lineTo(mLine[i + 1]);
		if (ps.createStroke(path).contains(pos) && i != 0 && (i + 1 != mLine.size() - 1)) { // вот тут разобратьс€, будет ли "лишнее вхождение" одной точки в 2 сегмента
