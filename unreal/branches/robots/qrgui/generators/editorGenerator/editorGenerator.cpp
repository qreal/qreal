...
//	QString const unicodeValue = codec ? codec->toUnicode(value.toStdString().c_str()) : value;
	QString const unicodeValue = value;
	if ((unicodeValue == "") && ((tagName == "displayedName")))
		return;
	if (unicodeValue == "") {
		...
		if (patten.exactMatch(value))
			element.setAttribute(tagName, unicodeValue);
		else {
			mErrorReporter.addWarning("wrong name\n",id);
			element.setAttribute(tagName, unicodeValue);
		}
	}
	else element.setAttribute(tagName, unicodeValue);
}
