...
VisualDebugger::ErrorType VisualDebugger::checkEditor() {
	...
	if (editorName.compare("(Robot Behavior Diagram)") != 0) {
...
void VisualDebugger::doStep(Id id) {
	...
	if (elem) {
		if (elem->uuid().element() == "EnginesForward"
				|| elem->uuid().element() == "EnginesBackward"
				|| elem->uuid().element() == "EnginesStop"
				|| elem->uuid().element() == "Timer"
				|| elem->uuid().element() == "Sensor"
		) {
			processAction();
		}
	}
