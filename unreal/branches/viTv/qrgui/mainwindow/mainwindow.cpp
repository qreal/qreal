...
#include "../generators/viTv/viTvGenerator.h"
	...
	: mListenerManager(NULL), mPropertyModel(mEditorManager)
{
	...
	connect(ui.actionGenerate_viTv_state_machine, SIGNAL(triggered()), this, SLOT(generateViTvStateMachine()));
...
void MainWindow::generateViTvStateMachine()
{
	generators::ViTvGenerator viTvGenerator(mModel->api());

	gui::ErrorReporter const errors = viTvGenerator.generate();
	errors.showErrors("Generation finished successfully", ui.ErrorListWidget);
