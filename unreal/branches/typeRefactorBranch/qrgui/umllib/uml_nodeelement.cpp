...
void NodeElement::arrangeLinksRecursively(QSet<NodeElement*>& toArrange, QSet<NodeElement*>& arranged)
{
	...
	foreach (StatLine line, mLinePorts) {
		...
		foreach (EdgeElement* edge, mEdgeList) {
			if (portId(edge->portIdOn(this)) == lpId) {
				...
				sortedEdges.insertMulti(qMakePair(scalarProduct, edge->portIdOn(this)), edge);
                                qDebug() << "+" << edge->newType().toString() <<"pr=" <<scalarProduct << "; p=" << edge->portIdOn(this);
		...
		foreach (EdgeElement* edge, sortedEdges) {
                        qDebug() << "-" << edge->newType().toString() <<lpId + (1.0 + i) / (N + 1);
...
bool NodeElement::initPossibleEdges()
{
	...
	foreach(QString elementName,
                        itemModel->assistApi().editorManager().getEditorInterface(newType().editor())->elements(newType().diagram())) {
                int ne = itemModel->assistApi().editorManager().getEditorInterface(newType().editor())->isNodeOrEdge(elementName);
		if (ne == -1) {
			QList<StringPossibleEdge> list
                                        = itemModel->assistApi().editorManager().getEditorInterface(newType().editor())->getPossibleEdges(elementName);
