...
void EmbeddedLinker::initTitle()
{
	EditorManager* editorManager = dynamic_cast<EditorViewScene*>(scene())->mainWindow()->manager();
        QString edgeTypeFriendly = editorManager->friendlyName(NewType::loadFromString("qrm:/"+master->newType().editor()+"/"+master->newType().diagram()+"/"+edgeType.element()));
...
void EmbeddedLinker::setEdgeType(const qReal::NewType &arg)
{
	edgeType = arg;
	generateColor();
}
...
qReal::NewType EmbeddedLinker::getEdgeType()
{
	return edgeType;
}
...
void EmbeddedLinker::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
	...
	if (scene)
	{
                const QString type = "qrm:/" + master->newType().editor() + "/" +
                                                         master->newType().diagram() + "/" + edgeType.element();
