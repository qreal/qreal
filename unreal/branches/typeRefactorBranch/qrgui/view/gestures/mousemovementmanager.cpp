...
MouseMovementManager::MouseMovementManager(QList<qReal::NewType> elements, qReal::EditorManager * editorManager,
...
void MouseMovementManager::drawIdealPath()
{
	QString currentElement = mGesturesPaintMan->currentElement();
        foreach (qReal::NewType element, mGestures.values())
	{
...
{
	mGestures.clear();
        foreach (qReal::NewType element, mElements)
	{
...
void MouseMovementManager::setElements(const QList<qReal::NewType> &elements)
{
	mElements = elements;
	createMap();
}
...
qReal::NewType MouseMovementManager::getObject()
{
	const float e = 100;
