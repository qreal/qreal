...
{
    Q_FOREACH (ModelTreeItem *child, root->children()) {
        mApi.removeElement(child->id());
        removeModelItems(child);
        int childRow = child->row();
...
{
    Q_FOREACH (Id childId, mApi.children(parent->id())) {
...
void Model::checkProperties(Id const &id)
{
    ...
    Q_FOREACH (QString property, propertiesThatShallBe)
...
{
    Q_FOREACH (ModelTreeItem *childItem, root->children()) {
