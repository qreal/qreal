...
void XmlParser::setPortsAttribute(const QDomElement &node, const Id &nodeId)
{
//    QDomNodeList graphics = node.childNodes();
//    for(unsigned i = 0; i < graphics.length(); ++i){
//            QDomElement element = graphics.at(i).toElement();
//            if (element.tagName() == "ports")
//            {
//                for (unsigned j = 0; j < element.childNodes().length(); ++j)
//                {
//                    QDomElement port = element.childNodes().at(j).toElement();
//                    QString portName = port.attribute("name", "");
//                    QString orderNumber = port.attribute("orderNumber", "");
//                    if (portName != "" && orderNumber != "")
//                    {
//                        mApi.setProperty(nodeId, portName, orderNumber);
//                    }
//                }
//            }
//    }
