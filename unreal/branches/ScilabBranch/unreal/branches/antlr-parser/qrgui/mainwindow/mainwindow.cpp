        ...
        : mListenerManager(NULL), mPropertyModel(mEditorManager)
{
    ...
    if (!missingPlugins.isEmpty()) {
        ...
        Q_FOREACH (Id const id, missingPlugins) {
            text += id.editor() + "\n";
        }
...
{
    Q_FOREACH (Id const editor, mEditorManager.editors()) {
        Q_FOREACH (Id const diagram, mEditorManager.diagrams(editor)) {
            ui.paletteToolbox->addDiagramType(diagram, mEditorManager.friendlyName(diagram) );

            Q_FOREACH (Id const element, mEditorManager.elements(diagram)) {
...
{
    Q_FOREACH (QGraphicsItem *item, getCurrentTab()->scene()->selectedItems())
