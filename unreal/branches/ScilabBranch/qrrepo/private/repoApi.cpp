...
Id RepoApi::linksByPort(Id const &id, double portOrderNumber) const
{
    IdList links = mClient.property(id, "links").value<IdList>();
    Id result;
    foreach (Id const link, links) {
        double portNumber = fromPort(link);
            if (portNumber ==portOrderNumber) {
                    result = link;
            }
    }
    return result;
...
double RepoApi::portOrderNumberByName(Id const &id, QString const &portName) const
{
    if (mClient.hasProperty(id, portName))
    {
        double orderNum = mClient.property(id, portName).toDouble();
        return orderNum;
    }
    return 0.0;
}
