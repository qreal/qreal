...
bool Model::dropMimeData( const QMimeData *data, Qt::DropAction action, int row, int column, const QModelIndex &parent )
{
	...
	if (action == Qt::IgnoreAction) {
		...
		if (treeItems.contains(id)) { // Пока контейнеры не работают, бросать существующие элементы нельзя.
			changeParent(index(treeItems.find(id).value()), parent, QPointF(0, 0));
				// TODO: Найти ПРАВИЛЬНЫЙ элемент для репарентинга. Пока - первый попавшийся.
			return true;
		}
...
void Model::changeParent(QModelIndex const &element, QModelIndex const &parent, QPointF const &position)
{
	...
	/*
	foreach (ModelTreeItem *child, elementItem->parent()->children()) {
		changePersistentIndex(index(child), блаблабла);
	}
	*/
	...
	QModelIndex newIndex = index(elementItem);
	changePersistentIndex(element, newIndex);
