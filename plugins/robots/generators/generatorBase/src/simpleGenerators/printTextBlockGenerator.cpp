...
PrintTextBlockGenerator::PrintTextBlockGenerator(qrRepo::RepoApi const &repo
	...
	: BindingGenerator(repo, customizer, id, "drawing/printText.t", QList<Binding *>()
			<< Binding::createConverting("@@X@@", "XCoordinateText", customizer.factory()->intPropertyConverter(id))
			<< Binding::createConverting("@@Y@@", "YCoordinateText", customizer.factory()->intPropertyConverter(id))
