		...
		, qReal::Id const elementId, qReal::Id const logicElementId)
{
	...
	for (int i = 1; i <= numberOfPorts; ++i) {
		qReal::interpreters::robots::sensorType::SensorTypeEnum portValue = trikGenerator->portValue(i);
	...
	trikGenerator->initCode().append(initCode);
	trikGenerator->terminateCode().append(terminateCode);
	trikGenerator->isrHooksCode().append(isrHooks);
