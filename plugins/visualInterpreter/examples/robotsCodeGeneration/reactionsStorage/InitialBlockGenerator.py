...
for i in xrange(numberOfPorts):
  ...
  if portValues[i] == "Ультразвуковой сенсор":
    initCode.append("ecrobot_init_sonar_sensor(NXT_PORT_S" + str(i) + ");\n")
    terminateCode.append("ecrobot_term_sonar_sensor(NXT_PORT_S" + str(i) + ");\n")
  elif portValues[i] == "Сенсор цвета (все цвета)":
    initCode.append(initEcrobotColorSensorPortS + "1, NXT_LIGHTSENSOR_WHITE);\n")
    terminateCode.append("ecrobot_term_nxtcolorsensor(NXT_PORT_S" + str(i) + ");\n")
