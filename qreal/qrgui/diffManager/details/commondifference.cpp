...
CommonDifference::CommonDifference(models::Models *oldModel, models::Models *newModel, Id const &id)
...
void CommonDifference::findCommonDifference()
{
	if (repoApi(true).exist(mId))
	{
		if (repoApi(false).exist(mId))
		{
			...
			mState = (mParentState == Same && mChildrenState == Same && mPropertiesState == Same) ? Same : Modified;
		}
		else
		{
...
void CommonDifference::findPropertyDifference()
{
	...
	while (i.hasNext())
	{
		...
		QVariant newValue = repoApi(false).property(mId, key);
		Modification modification;
		modification.setOldValue(oldValue);
		modification.setNewValue(newValue);
		DiffState state = (compareProperties(oldValue, newValue)) ? Same : Modified;
		if (state == Modified)
			hasModifications = true;
...
bool CommonDifference::compareProperties(const QVariant &property1, const QVariant &property2) const
{
